...
function MusicLibraryContent() {
  ...
  const renderSongList = () => {
    ...
    return (
      <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-5 stagger-children max-md:grid-cols-1">
        ...
        <main className="flex-1 w-full max-w-[1280px] mx-auto px-6 pb-16 pt-6 max-md:px-4 max-md:pb-12">
            ...
            <p className="text-sm text-slate-400 font-medium">
              {isLoading ? (
                ...
                <>
                  ...
                  {submittedTerm && (
                    <span className="text-indigo-400 italic"> matching &quot;{submittedTerm}&quot;</span>
                  )}
                  {localSongs.length > 0 && (
                    <span className="text-sky-400"> ({localSongs.length} local)</span>
                  )}
                </>
              )}
            </p>
          ...
          {isAuthenticated && !isAdmin && (
            <div className="mb-4 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-xs text-slate-500 animate-fade-in">
              👋 You&apos;re signed in as a <span className="text-slate-300 font-medium">viewer</span>. Sign in as <span className="text-sky-400 font-medium">admin</span> to add or remove songs.
            </div>
          )}
